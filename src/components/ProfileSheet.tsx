      ...
      <SheetContent side="right" className="w-full sm:max-w-lg">
        ...
        <ScrollArea className="h-[calc(100vh-180px)] mt-6">
          <div className="space-y-6 pr-4">
            ...
            {/* Photos Uploaded */}
            <div className="space-y-2">
              <Label className="font-light">Photos</Label>
              <div className="text-sm text-muted-foreground">
                {formData.facialPhotos?.length > 0 && (
                  <div>Facial photos: {formData.facialPhotos.length}</div>
                )}
                {formData.bodyPhotos?.length > 0 && (
                  <div>Body photos: {formData.bodyPhotos.length}</div>
                )}
                {formData.swimsuitPhotos?.length > 0 && (
                  <div>Swimsuit photos: {formData.swimsuitPhotos.length}</div>
                )}
