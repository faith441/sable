...
import { ArrowLeft, X, Heart, Link as LinkIcon, ThumbsDown, Share2, Sun } from "lucide-react";
...
const OutfitRecommendations = () => {
  ...
  useEffect(() => {
    ...
    if (savedOutfits) {
      try {
        const data = JSON.parse(savedOutfits);
        setOutfits(data);
      } catch (error) {
      ...
      // No outfits found, go back to chat
      navigate('/ai-style-chat');
    }
  }, [navigate]);
      ...
      <div className="flex-1 overflow-y-auto overscroll-behavior-contain">
        <div className="max-w-2xl mx-auto px-4 pb-6">
          ...
          <div className="flex items-center justify-between py-4 gap-2 border-b border-gray-100">
            ...
            {/* Create Avatar Button - Mobile Optimized */}
            <button
              onClick={() => {
                localStorage.setItem('virtual-tryon-outfit', JSON.stringify(currentOutfit));
